...
def show_detections_with_ground_truth(detections, scores, file_names, params: Parameters):
    ...
    for test_file in test_files:
        ...
        for idx, detection in enumerate(current_detections):
            detection_0 = int(detection[0])
            detection_1 = int(detection[1])
            detection_2 = int(detection[2])
            detection_3 = int(detection[3])
            cv.rectangle(image, (detection_0, detection_1), (detection_2, detection_3), (0, 0, 255), thickness=1)
            cv.putText(image, 'score:' + str(current_scores[idx])[:4], (detection_0, detection_1),
                       cv.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 1)
